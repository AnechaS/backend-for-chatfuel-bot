  ...
  const handleSubmit = useCallback(
    async ({ password }, { setStatus }) => {
      try {
        ...
        // get session token
        const sessionToken = await login(user.email, password).then(
          ({ data }) => data.sessionToken
        );
        
        dispatch(auth.actions.changeToken(sessionToken));
        ...
        console.log(error);
        setStatus("การส่งคำร้องข้อ เปลียนรหัสผ่านเข้าสู่ระบบล่มเหลว");
      } finally {
        // spinner off
        setLoading(false);
      }
